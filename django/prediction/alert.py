...
class XGBModelPredictor:
    ...
    def predict_weekly_income(self, custom_data=None):
        ...
        try:
            ...
            # 检查是否触发预警
            alert_triggered = check_and_alert('weekly', prediction)
    ...
    def predict_monthly_income(self, custom_data=None):
        ...
        try:
            ...
            # 检查是否触发预警
            alert_triggered = check_and_alert('monthly', prediction)
            result["alert_triggered"] = alert_triggered
...
def check_and_alert(forecast_type, predicted_value, threshold=0.1):
    """
    检查预测偏差并触发预警（需结合数据库记录）
    """
    try:
        # 获取历史预测记录（最近一条同类型预测）
        ...
        historical_value = latest_record.predicted_value  # 暂用历史预测值
        
        # 计算偏差
        deviation = abs(predicted_value - historical_value) / historical_value
        
        # 触发预警条件
        if deviation > threshold:
            # 发送预警邮件
            subject = f"【预警】{forecast_type.capitalize()}预测偏差超阈值"
            message = f"""
            预测类型: {forecast_type}
            预测值: {predicted_value}
            历史值: {historical_value}
            偏差百分比: {deviation:.2%}（阈值: {threshold:.2%}）
            """
            send_alert_status = send_alert(subject, message)
            
            # 更新数据库记录
            latest_record.is_alerted = send_alert_status
            latest_record.alert_deviation = deviation
            latest_record.save()
        ...
        print("❌ 无历史预测记录，无法计算偏差")
        return False
    except Exception as e:
        print(f"预警检查失败: {str(e)}")
        return False

def send_alert(subject, message, recipient=settings.EMAIL_HOST_USER):
    """发送预警邮件（需提前配置邮箱设置）"""
    try:
        send_mail(
            subject=subject,
